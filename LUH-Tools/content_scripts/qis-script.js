...
function mainQISCalculator() {
    ...
    for (var i = 0, row; row = table.rows[i]; i++) {
        ...
        if(document.documentElement.lang === "de") { //detect if QIS side language 
    ...
    insertSummary("luh-tool-lp", chrome.i18n.getMessage("total_credit_points"), lp_count);
    insertSummary("luh-tool-weighted", chrome.i18n.getMessage("total_credit_points_weighted"), lp_weighted);
    insertSummary("luh-tool-grade", chrome.i18n.getMessage("weighted_grade"), rounded_grade);
    insertSummary("luh-tool-average-grade", chrome.i18n.getMessage("average_grade"), rounded_average_grade);
}
...
function mainQISCalculatorV2() {
    ...
    let name_must_not_include = (document.documentElement.lang === "de" ? "module" : "modules");
    ...
    let rows_filtered = [...grade_table.rows].filter((row) => {
        ...
        if (!(cells[0].classList.contains("qis_konto") || cells[0].classList.contains("mod_n"))) return false; //skips irrelevant rows
    ...
    for(let row of rows_filtered) {
        ...
        stats.lp_count += lp;
        if (!isNaN(grade)) { //if row has a grade
    ...
    let rounded_average_grade = (stats.sum_grade/stats.total_grade_count).toFixed(3);
    ...
    insertSummary("luh-tool-lp", chrome.i18n.getMessage("total_credit_points"), stats.lp_count);
    insertSummary("luh-tool-weighted", chrome.i18n.getMessage("total_credit_points_weighted"), stats.lp_weighted);
    insertSummary("luh-tool-grade", chrome.i18n.getMessage("weighted_grade"), rounded_grade);
    insertSummary("luh-tool-average-grade", chrome.i18n.getMessage("average_grade"), rounded_average_grade);
}